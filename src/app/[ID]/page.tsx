...
export default function JobInfoPage({ params }: JobInfoPageProps) {
  ...
  const idJob = params.ID.match(/\d+$/)?.[0];

  const selectedOffer = offers.find((job: { ref_number: string | undefined; }) => job.ref_number === idJob);
  const { title, description, requirements, annotation } = selectedOffer || {};
  const location = offers.find((job: any) => {
    const { city, state, branch, custom_fields, pub_date } = job;
    const resposta = custom_fields?.[0]?.resposta || "Presencial";
    return `${city} - ${state} | ${branch} | ${resposta} | ${pub_date}`;
  });

  if (!selectedOffer && !loading) {
    redirect('/');
  }
  ...
  return (
    <section>
      <img
        src="https://wellowgroup.com/wp-content/uploads/2022/06/Wellow_Site_Background_Frase-01.jpg"
        alt="Wellow Background"
        className='h-[32vh] w-full object-cover'
      />
      <div className="flex justify-start flex-start ml-[10%] translate-y-[-9%] max-[1199px]:items-center max-[1199px]:ml-0 max-[1199px]:flex-col max-[500px]:translate-y-[-26vh]">
        {loading ? (<Skeleton className='paper loading flex flex-col gap-[40px] p-[3rem] justify-center items-start' />) : (
          <div className="paper bg-white flex flex-col gap-[40px] w-[63rem] max-[1199px]:w-[80vw] max-[809px]:w-[100vw] p-[3rem] justify-center items-start">
            <h1 className="tituloPaper">{title}</h1>
            <div className="w-full">
              <p>{location && `${location.city} - ${location.state} | ${location.branch} | ${location.custom_fields?.[0]?.resposta || "Presencial"} | ${location.pub_date}`}</p>
